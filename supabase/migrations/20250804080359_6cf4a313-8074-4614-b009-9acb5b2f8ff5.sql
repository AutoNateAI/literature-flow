...
-- Add a new check constraint that includes notebook and source types
ALTER TABLE graph_nodes ADD CONSTRAINT graph_nodes_node_type_check 
CHECK (node_type = ANY (ARRAY['concept'::text, 'hypothesis'::text, 'gap'::text, 'discrepancy'::text, 'publication'::text, 'insight'::text, 'notebook'::text, 'source'::text]));
...
-- Now create graph nodes for all existing notebooks that don't have them
...
WHERE NOT EXISTS (
  ...
  AND gn.resource_id = nr.id
);

-- Create functions and triggers for automatic node creation
...
DROP TRIGGER IF EXISTS trigger_create_notebook_graph_node ON notebooks;
CREATE TRIGGER trigger_create_notebook_graph_node
  AFTER INSERT ON notebooks
  FOR EACH ROW
  EXECUTE FUNCTION create_notebook_graph_node();

DROP TRIGGER IF EXISTS trigger_create_resource_graph_node ON notebook_resources;
